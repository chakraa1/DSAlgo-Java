...
public class LongestNiceSubstring {
	...
	private String longestNiceSubstringDivideConquer(String input) {
		...
		for(int i=1;i<input.length();i++) {
			...
			if((Character.isLowerCase(c) && set.contains(Character.toUpperCase(c))) || (Character.isUpperCase(c) && set.contains(Character.toLowerCase(c)))) {
				continue;
			}
			...
			return previousLongerSubstring.length() >=nextLongerSubstring.length() ? previousLongerSubstring: nextLongerSubstring;
	...
	private String longestNiceSubstringSlidingWindow(String input) {
		
		int j = 0;
		int windowSize=2;int n=input.length();
		...
		for (int i = 0; i < n && j < n; i++) {
			...
			if(!isNice(substring)) {
				...
				niceSubstringMap.put(substring, previousNiceSubstringLastIndex > 0 ? j - previousNiceSubstringLastIndex:0);
	...
	private boolean  isNice(String substring) {
		...
		if((Character.toLowerCase(firstChar) == Character.toLowerCase(lastChar)) && ((Character.isLowerCase(firstChar) && Character.isUpperCase(lastChar)) || (Character.isUpperCase(firstChar) && Character.isLowerCase(lastChar)))) {
			return true;
		}
	...
	public static void main(String[] args) { 
		...
		/*
		 * Example 1: s = "YazaAay" Output: "aAa"
		 * Example 2: s = "Bb" Output: "Bb"
		 * Example 3: s = "c" Output: "" 
		 * Example 3: s = "dDzeE" Output: "dD" 
		 */
		...
		System.out.println("LongestNiceSubstring (SlidingWindow Approach) -> "+soln.longestNiceSubstringSlidingWindow(input));
	}
